...
const MovieDetailsAnalyticsModel = require("../../models/CentralizedMoviesCreateModels/MovieDetailsAnalyticsModel");
...
exports.startMovieSession = async (req, res) => {
  ...
  try {
    ...
    const existingSession = await MovieDetailsAnalyticsModel.findOne({
      where: {
        movieId,
        sessionId,
      },
    ...
    if (existingSession) {
      ...
      return res.status(200).json({
        ...
        analyticsId: existingSession.id,
        message: "Session already exists",
      });
    }

    // =====================================
    // YOUTUBE STYLE DUPLICATE VIEW
    // SAME VISITOR + SAME MOVIE
    // WITHIN 30 MINUTES
    // =====================================
    const thirtyMinutesAgo = new Date(Date.now() - 30 * 60 * 1000);

    const existingView = await MovieDetailsAnalyticsModel.findOne({
      where: {
        movieId,
    ...
    if (existingView) {
      ...
      return res.status(200).json({
        success: true,
        duplicateView: true,
        analyticsId: existingView.id,
        message: "View already counted in last 2 hours",
      });
    }
...
exports.endMovieSession = async (req, res) => {
  ...
  try {
    ...
    let timeSpent = Math.floor((endTime - new Date(sessionStartAt)) / 1000);
    // Negative prevent
